...
class HardDetectionModule(nn.Module):
    def __init__(self, edge_threshold=5):
        ...
        self.dii_filter = torch.tensor(  # Hessian Detector
            [[0, 1., 0], [0, -2., 0], [0, 1., 0]]
        ).view(1, 1, 3, 3)
    ...
    def forward(self, batch):
        # 在训练的 SoftDetectionModule 基本过程就是, 找到在深度和临近9像素内都为最大值的点
        b, c, h, w = batch.size()  # b = 1, c = 512, h = 63, w = 63
        device = batch.device

        # 找到深度上的最大值
        depth_wise_max = torch.max(batch, dim=1)[0]  # (1, 63, 63)
        # (1, 512, 63, 63) 值为 0 或 1
        is_depth_wise_max = (batch == depth_wise_max)
        del depth_wise_max

        # (1, 512, 63, 63) 找到临近 9 像素内的最大值
        local_max = F.max_pool2d(batch, 3, stride=1, padding=1)
        is_local_max = (batch == local_max)  # (1, 512, 63, 63)
        del local_max

        # batch.view(-1, 1, h, w) 会变成 (512, 1, 63, 63)
        # 类比的是 512 张 63*63 大小的单通道图
        dii = F.conv2d(
        ...
        is_not_edge = torch.min(
            tr * tr / det <= threshold,
            det > 0)  # 得到边缘检测的结果

        # 三者中全部为真, 即深度最大, 且局部最大, 且不是边缘, 才可以看做是关键点.
        detected = torch.min(
            is_depth_wise_max,
...
class HandcraftedLocalizationModule(nn.Module):
    ...
    def forward(self, batch):
        # batch 应当是 dense_features
        b, c, h, w = batch.size()
        device = batch.device

        dii = F.conv2d(  # (1, 512, 63, 63)
            batch.view(-1, 1, h, w), self.dii_filter.to(device), padding=1
        ).view(b, c, h, w)
        dij = F.conv2d(  # (1, 512, 63, 63)
            batch.view(-1, 1, h, w), self.dij_filter.to(device), padding=1
        ).view(b, c, h, w)
        djj = F.conv2d(  # (1, 512, 63, 63)
            batch.view(-1, 1, h, w), self.djj_filter.to(device), padding=1
        ).view(b, c, h, w)
        det = dii * djj - dij * dij

        inv_hess_00 = djj / det  # (1, 512, 63, 63)
        inv_hess_01 = -dij / det
        inv_hess_11 = dii / det
        del dii, dij, djj, det
